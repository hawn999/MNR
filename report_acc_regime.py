...
def init_acc_regime(dataname):
    ...
    elif 'Unicode' in dataname:
        return init_acc_regime_unicode()
    elif 'MNR' in dataname:
        return init_acc_regime_mnr()
    else:
...
def update_acc_regime(dataname, acc_regime, model_output, target, structure_encoded, data_file):
    if 'RAVEN' in dataname:
        ...
        update_acc_regime_unicode(acc_regime, model_output, target, data_file)
    elif 'MNR' in dataname:
        update_acc_regime_mnr(acc_regime, model_output, target, data_file)
    else:
        update_acc_regime_mnr(acc_regime, model_output, target, structure_encoded)
...
def init_acc_regime_unicode():
    acc_regime = {"Ar-N-N-N-H": [0, 0],
                  "Co-N-N-N-H": [0, 0],
                  "Di-N-N-N-H": [0, 0],
                  "Pr-N-N-N-H": [0, 0],
                  "Un-N-N-N-H": [0, 0],
                  }
    # acc_regime = {"Ar-N-D-N-H": [0, 0],
    #               "Co-N-D-N-H": [0, 0],
    #               "Di-N-D-N-H": [0, 0],
    #               "Pr-N-D-N-H": [0, 0],
    #               "Un-N-D-N-H": [0, 0],
...
def init_acc_regime_mnr():
    acc_regime = {"combination_analytical": [0, 0],
                  "combination_holistic": [0, 0],
                  "partition_analytical": [0, 0],
                  "partition_holistic": [0, 0],
                  "composition_analytical": [0, 0],
                  "composition_holistic": [0, 0],
                  }
    return acc_regime

def update_acc_regime_mnr(acc_regime, model_output, target, data_file):
    # def update_acc_regime_raven(acc_regime, model_output, target, data_file):
    #     acc_one = model_output.data.max(1)[1] == target
    #     for i in range(model_output.shape[0]):
    #         regime = data_file[i].split('\\' if os.name == 'nt' else '/')[1]
    #         # 若是 SRAVEN 或其他新数据集，没有对应键就自动建一个
    #         if regime not in acc_regime:
    #             acc_regime[regime] = [0, 0]
    #         acc_regime[regime][0] += acc_one[i].item()
    #         acc_regime[regime][1] += 1
    acc_one = model_output.data.max(1)[1] == target
    for i in range(model_output.shape[0]):
        regime = data_file[i].split('\\' if os.name == 'nt' else '/')[1]
        regime = regime.split('_')[2] +"_"+ regime.split('_')[6]
        acc_regime[regime][0] += acc_one[i].item()
        acc_regime[regime][1] += 1

def update_acc_regime_unicode(acc_regime, model_output, target, data_file):
    acc_one = model_output.data.max(1)[1] == target
    for i in range(model_output.shape[0]):
