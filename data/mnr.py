...
class MNR(Dataset):
    def __init__(
        ...
        self.file_names = []
        # for i in subsets:
        file_names = [f for f in os.listdir(self.dataset_dir+"/"+data_split+"_set")]
        self.file_names += [os.path.join(data_split+"_set", f) for f in file_names]
    ...
    def _get_data(self, idx):
        ...
        context_images = data["context_images"].reshape(3,80,80)
        answer_images = data["answer_set_images"].reshape(8,80,80)
        image = np.concatenate([context_images, answer_images], axis=0)

        if self.image_size != 80:
            resize_image = np.zeros((11, self.image_size, self.image_size))
    ...
    def __getitem__(self, idx):
        ...
        # Get additional data
        target = data["correct_answer_image_index"]
        del data
