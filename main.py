...
def main_worker(args):
    ...
    #          use_muon=True, lr=0.005, weight_decay=args.weight_decay),
    #     dict(params=[p for name, p in model.named_parameters() if p not in classifier_params and p.ndim < 2],
    ...
    for i, (images, target, meta_target, structure_encoded, data_file) in enumerate(data_loader):
        ...
        if args.fp16:
            with torch.cuda.amp.autocast():
                ...
                # # # v2
                # aux = getattr(model.module, 'aux_losses', getattr(model, 'aux_losses', None))
                # if isinstance(aux, dict):
                #     w_dag = getattr(args, 'dag_loss_weight', 0.0)
                #     w_l1 = getattr(args, 'graph_l1_weight', 0.0)
                #     w_ent = getattr(args, 'mask_ent_weight', 0.0)
                #
                #     def mean_if_tensor(x):
                #         if x is None: return None
                #         if torch.is_tensor(x) and x.dim() > 0: return x.mean()
                #         return x
                #
                #     if 'acyclicity' in aux and aux['acyclicity'] is not None:
                #         loss = loss + w_dag * mean_if_tensor(aux['acyclicity'])
                #     if 'A_l1' in aux and aux['A_l1'] is not None:
                #         loss = loss + w_l1 * mean_if_tensor(aux['A_l1'])
                #     if 'mask_entropy' in aux and aux['mask_entropy'] is not None:
                #         loss = loss + w_ent * mean_if_tensor(aux['mask_entropy'])
