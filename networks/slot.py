...
from .network_utils import (
    ...
    LinearNormAct, convert_to_rpm_matrix_v15
)
from .position_embedding import PositionalEncoding, LearnedAdditivePositionalEmbed
from .predrnet_original_source_code import original_PredictiveReasoningBlock
...
class PredictionInterAttention(nn.Module):
    ...
    def forward(self, x, atten_flag):
        ...
        x_out = torch.cat([x_att[:, :, :self.num_contexts - 1], p], dim=2)
        return x_out
        return self.drop2(x_out)
    ...
    def __init__(
        ...
        for l in range(self.num_giprbs):
            setattr(
                ...
                # self, "prb" + str(l),
                # original_PredictiveReasoningBlock(in_planes, ou_planes,
...
class PredRNet(nn.Module):
    ...
    def forward(self, x, train=False):
        ...
        elif self.in_channels == 3:#B,16,3,80,80
            b, n, _, h, w = x.size()
...
def predrnet_raven_graph(**kwargs):
