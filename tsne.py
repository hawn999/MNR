...
# ablation
parser.add_argument('--reduce-planes', default=160, type=int,
...
# def visualize_random_samples(loader, device, n_samples=3):
#     """
#     随机抽取 n_samples 道题：
#     第 1 行：context（通常 8 张）
#     第 2 行：8 个候选答案（标题里标出正确答案索引）
#     """
#     import random
#     import matplotlib.pyplot as plt
...
#     import torch
#
#     images, targets, *_ = next(iter(loader))          # 取 1 个 batch
#     images, targets = images.to(device), targets.to(device)
#
#     B, N, H, W = images.shape                         # N = context + 8
#     context_cnt = N - 8                               # 支持 MNR(3) 或 RAVEN(8)
#
#     idxs = random.sample(range(B), min(n_samples, B))
#     fig_h = 2.8 * len(idxs)
#     fig, axes = plt.subplots(len(idxs), 1, figsize=(12, fig_h))
#
#     if len(idxs) == 1:                               # 保证 axes 可迭代
#         axes = [axes]
#
#     for ax, idx in zip(axes, idxs):
#         sample = images[idx]                         # (N, H, W)
#         tgt     = targets[idx].item()
#
#         ctx_grid = vutils.make_grid(
#             sample[:context_cnt].unsqueeze(1),        # (C=1)
#             nrow=context_cnt, padding=2,
#             normalize=True, scale_each=True
#         )
#         ans_grid = vutils.make_grid(
#             sample[context_cnt:].unsqueeze(1),
#             nrow=8, padding=2,
#             normalize=True, scale_each=True
#         )
#         # 垂直拼接：先 pad 宽度取最大
#         if ctx_grid.size(2) != ans_grid.size(2):
#             diff = abs(ctx_grid.size(2) - ans_grid.size(2))
#             pad  = (0, diff, 0, 0) if ctx_grid.size(2) < ans_grid.size(2) else (0, diff, 0, 0)
#             ctx_grid = torch.nn.functional.pad(ctx_grid, pad) if ctx_grid.size(2) < ans_grid.size(2) else ctx_grid
#             ans_grid = torch.nn.functional.pad(ans_grid, pad) if ans_grid.size(2) < ctx_grid.size(2) else ans_grid
#
#         full = torch.cat([ctx_grid, ans_grid], dim=1)  # 上下堆叠
#         ax.imshow(full.permute(1, 2, 0).cpu().numpy(), cmap="gray")
#         ax.set_title(f"Sample #{idx} – target={tgt}")
#         ax.axis("off")
#
#     plt.tight_layout()
#     plt.show()
def extract_and_plot_tsne(val_loader, model, args):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device).eval()

    giprb_feats, hyla_feats, labels = [], [], []

    # 注册 forward hook
    for i in range(4):
        getattr(model.module.MAutoRR0, f"GIPRB{i}").register_forward_hook(
            lambda m, _, o: giprb_feats.append(o.detach().reshape(o.shape[0], -1).cpu())
        )
    for i in range(3):
        getattr(model.module.MAutoRR0, f"hyla{i}").register_forward_hook(
            lambda m, _, o: hyla_feats.append(o.detach().reshape(o.shape[0], -1).cpu())
        )

    with torch.no_grad():
        for i, (images, target, *_rest) in enumerate(val_loader):
            if i >= 10:  # 控制最大batch数
                break
    ...
    # 拼接
    giprb_mat = torch.cat(giprb_feats, dim=0)
    hyla_mat = torch.cat(hyla_feats, dim=0)
    labels = torch.cat(labels, dim=0)

    def run_tsne(mat):
        tsne = TSNE(n_components=2, perplexity=30, random_state=42, init="pca")
        return tsne.fit_transform(mat.numpy())

    def plot_tsne(data2d, labels, title, save_path):
        plt.figure(figsize=(6, 5))
        sns.scatterplot(x=data2d[:, 0], y=data2d[:, 1], hue=labels, palette="tab10", s=18, legend=False)
        plt.title(title)
        plt.tight_layout()
    ...
    # 可视化
    giprb_2d = run_tsne(giprb_mat)
    hyla_2d = run_tsne(hyla_mat)
    plot_tsne(giprb_2d, labels, "GIPRB features (t-SNE)", "giprb_tsne.png")
    plot_tsne(hyla_2d, labels, "HYLA features (t-SNE)", "hyla_tsne.png")
...
def main_worker(args):
    ...
    if args.evaluate:
        extract_and_plot_tsne(vl_loader, model, args)
